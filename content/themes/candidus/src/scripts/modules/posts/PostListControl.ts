...
export default class PostListControl extends Control implements ControlStruct {
  /**
   * @desc Container for the grid post layout
   */
  gridButton: PostListButton;
  /**
   * @desc Container for the feed post layout
   */
  ...
  /**
   * @desc The active list state either 'grid' or 'feed'
   */
  activeListState: ListState;
  /**
   * @desc The device object which is used to get notified about screen size changes
   */
  device: UserDevice;
  ...
  private initDeviceObserver() {
    const setFeedOnMobileDevices = (deviceType: DeviceType) => {
      const isSmallerDevice =
        this.device.getDeviceType() === 'mobile-sm' || this.device.getDeviceType() === 'mobile-md';
      if (isSmallerDevice) {
